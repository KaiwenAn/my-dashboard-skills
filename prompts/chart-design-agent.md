  ...
  "chart_list": [
    {
      ...
      "chart_type": "{{chart_type_enum}}（枚举，必填，必须严格使用 BI 平台中文名）",
...
> ⚠️ `chart_type` 字段**必须严格使用下表的"图表类型"中文名**。任何不在表中的名字都会被
> ChartDesignAgent.validate_output 拒收，导致整轮重新生成。
>
> 已知会被拒收的旧名字（不要再写）：「指标卡片」「柱状图」「数据表格」「指标趋势图」
> 「环形图」「堆叠柱状图」「面积图」「双轴图」。BI 平台对应的正确名字见下表。

{{chart_catalog}}

### 选型小提示

- 单个 KPI 数字（含同比/环比）→ `数字卡片`
- KPI + 趋势迷你线 / 多指标趋势对比 → `指标趋势卡`
- 时间序列趋势 → `折线图`
- 排名（项多时）→ `条形图`；项少且想强调高度 → `柱形图`
- 占比（分类 ≤ 6）→ `饼图`；分类多 → `条形图` 或 `矩形树图`
- 多指标多维度对比 → `雷达图`
- 双轴指标（一条趋势一条量级差异大的）→ `组合图`
- 行/列交叉汇总 → `透视表`；纯明细 → `表格`
- 高频词 / 关键词突出 → `词云图`
- 地理分布 → `地图`
- 流程转化阶段 → `漏斗图`
...
### Step 2：分配图表到分析层级
- **总览层（overview）**：
  - 核心指标用数字卡片，1-4个，放在最上方
  - 趋势用折线图/指标趋势卡，1-2个
  - 总览层图表不宜超过 6 个，保持一屏可见
- **下钻层（drilldown）**：
  - 按维度拆解用柱形图/条形图
  - 构成分析用饼图（≤6 分类）或矩形树图（层次结构）
  - 多指标多维度对比用雷达图
  - 双轴指标对比用组合图
  - 地域分布用地图
- **明细层（detail）**：
  - 用表格展示原始数据；行/列交叉汇总用透视表
  - 支持排序和翻页

### Step 3：选择图表类型
- 对每个分析需求，根据上面的"图表类型目录"选择最合适的类型
- 选型可参考"选型小提示"段落
- **常见反模式（必须避免）**：
  - ❌ 饼图超过 6 个分类 → 改用条形图或矩形树图
  - ❌ 折线图超过 5 条线 → 改用分面或筛选
  - ❌ 柱形图超过 15 根柱子 → 改用条形图或加筛选
  - ❌ 单指标无维度用折线图 → 改用数字卡片
  - ❌ 趋势数据用柱形图 → 改用折线图
...
```json
{
  "chart_list": [
    ...
    },
    {
      ...
      "design_notes": "类目排名用条形图（项多时优于柱形图），Top10，点击可联动地区分布"
    },
    {
      "chart_id": "chart_06",
      "chart_name": "各地区GMV分布",
    ...
    },
    {
      ...
      "interaction": {},
      "design_notes": "明细表格，支持排序和翻页，放在最底部"
    }
  ],
  "global_filters": [
    {
  ...
  "new_confirmation_items": [
    {
      "category": "图表类型",
      "item": "新客占比展示方式：当前用数字卡片展示新客数+支付用户数，是否需要直接展示占比百分比",
      "risk_if_wrong": "如果用户期望直接看到百分比数字，当前方案需要心算",
      "suggested_value": "建议数字卡片显示占比（新客数/支付用户数），hover 显示具体人数"
    }
  ]
}
```
